...
//REFACTORED VERSION OF ADVENTURER
int playAdventurer(struct gameState *state,int drawntreasure, int currentPlayer, int *temphand, int z,int cardDrawn){
	...
	if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
	  drawntreasure++;		//[BUG] Player continues to draw treasure... [updated] uncommented line of code
	else{
...
//REFACTORED VERSION OF SMITHY
int playSmithy(struct gameState *state, int handPos, int currentPlayer){
	int i;
	for (i = 0; i < 3; i++){//[BUG] Infinite drawing .. [updated] added 'i<3'
	  drawCard(currentPlayer, state);
	}
...
//REFACTORED VERSION OF VILLAGE
int playVillage(struct gameState *state, int currentPlayer,int handPos){
	drawCard(currentPlayer, state);         	
 	state->numActions = state->numActions + 2;
	
	//[BUG] Developer forgot to discard the card after use. Infinite Actions
	//discardCard(handPos, currentPlayer, state, 0);
	return 0;
}
//REFACTORED VERSION OF GREAT HALL
...
//REFACTORED VERSION OF COUNCIL ROOM
int playCouncilRoom(struct gameState *state, int currentPlayer,int handPos){
	...
	for (i = 0; i < 4; i++){
	  ...
	  if ( i == currentPlayer ){//[BUG] Only the current player draws
	      drawCard(i, state);
	    }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case adventurer:
	//REFACTORED FUNCTION
	playAdventurer(state,drawntreasure,currentPlayer,temphand,z,cardDrawn);    
	return 0;
